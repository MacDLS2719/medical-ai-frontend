...
function getSourceKey(sourceType) {
  ...
  if (t.includes('clinical')) return 'clinical_trials';
  if (t.includes('cochrane')) return 'cochrane';
  return null;
}
  ...
  return (
    <div className="mt-5 pt-4 border-t border-slate-100">
      <div className="font-semibold text-slate-700 mb-3 flex items-center gap-2 text-sm">
        ...
        Fuentes consultadas
        <span className="ml-1 text-xs font-normal text-slate-400">({documents.length} documentos encontrados)</span>
      </div>
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      role: 'assistant',
      content: 'Hola, soy tu asistente médico con IA. Pregúntame sobre patologías, tratamientos o evidencia clínica y buscaré en PubMed, ClinicalTrials.gov y Cochrane para darte una respuesta fundamentada.',
      documents: []
    }
  ]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const assistantMessage = {
        id: Date.now() + 1,
        role: 'assistant',
        content: data.response || data.reply || 'Lo siento, hubo un problema al procesar tu solicitud.',
      ...
      setMessages((prev) => [...prev, {
        id: Date.now() + 1,
        role: 'assistant',
        content: 'Hubo un error al conectar con el servidor. Por favor, intenta de nuevo más tarde.',
        documents: []
      }]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="flex-1 flex flex-col p-4 md:p-8 animate-in fade-in duration-500 h-full max-h-screen">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Asistente Médico IA</h1>
          <p className="text-slate-500 mt-1">RAG sobre PubMed · ClinicalTrials.gov · Cochrane</p>
        </div>
      </div>
      ...
      <div className="flex-1 glass-card rounded-3xl flex flex-col overflow-hidden border border-white/60 shadow-xl relative min-h-[500px]">
                ...
                <div className={`px-6 py-4 rounded-2xl shadow-sm text-[15px] leading-relaxed ${
                  ...
                  {msg.role === 'assistant' && msg.id !== 1 && (
                    <div className="mb-3 pb-2 border-b border-slate-100 font-semibold text-slate-700 flex items-center gap-2 text-sm">
                      <Bot size={15} className="text-indigo-500" /> Respuesta de IA
                    </div>
                  )}
            ...
            <div className="flex justify-start">
              <div className="flex gap-4 flex-row">
                ...
                <div className="px-6 py-4 rounded-2xl bg-white border border-slate-100 text-slate-500 rounded-tl-sm flex items-center gap-3 shadow-sm">
                  ...
                  <span className="animate-pulse">Buscando en PubMed, ClinicalTrials y Cochrane…</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-4 bg-white/80 border-t border-slate-100 backdrop-blur-md">
          ...
          <p className="text-center mt-2 text-xs text-slate-400 font-medium tracking-wide">
            La información generada por la IA no sustituye el diagnóstico de un profesional de la salud.
          </p>
        </div>
